...
def create_directory_structure():
    ...
    )  # Aller un niveau au-dessus
    data_dir = os.path.join(base_dir, "data")
    data_raw_dir = os.path.join(data_dir, "raw")
    data_processed_dir = os.path.join(data_dir, "preprocessed")  # Correction du dossier
...
def load_and_clean_data(data_raw_dir):
    ...
    reddit_df = reddit_df[
        (reddit_df["date"] >= start_date) & (reddit_df["date"] <= end_date)
    ]
    ...
    def clean_reddit_data(df):
        ...
        df["clean_text"] = (
            df["title"].apply(clean_text) + " " + df["text"].apply(clean_text)
        ).str.strip()
...
def save_cleaned_data(stock_df, reddit_df, data_processed_dir):
    try:
        stock_output, reddit_output = os.path.join(
            data_processed_dir, "clean_stock_data.csv"
        ), os.path.join(data_processed_dir, "clean_reddit_data.csv")
